...
import heart from "../../Assets/Gif/heart.png";
import heart1 from "../../Assets/Gif/heart1.png";
...
export default function DisplayProduct() {
  ...
  function handlewishcart(content) {
    switch (content) {
      case "addwishlist":
        setFav(true);
        ...
        <div className="slide_container">
          {fav ? (
            ...
            <img
              ...
              className="heart_img"
              onClick={(e) => {
                handlewishcart("addwishlist");
              }}
            />
          )}
          <button
            className="pointer left_arr"
            onClick={() => {
              handleslide("reduce");
            }}
          >
            {" "}
            <img src={arrow} alt="" className="arr1" />{" "}
          </button>
          <img
            className="image_size"
            ref={ImgRef}
            src={DisplayProd.images[imageIndex]}
            alt=""
          />
          ...
          {/* </div> */}
          <button
            className="pointer right_arr"
            onClick={() => {
              handleslide("add");
            }}
          >
            {" "}
            <img src={arrow} alt="" className="arr2" />
          </button>
        </div>
        <div className="text-start dp_bottom">
          <p>
            {DisplayProd.title} - {DisplayProd.brand}
          </p>
          <p>{DisplayProd.description}</p>
          <div className=" d-flex">
            ₹{DisplayProd.price} &nbsp;{" "}
            <sub className="text-line light_black">
              ₹
              {parseInt(
                DisplayProd?.price / (1 - DisplayProd.discountPercentage / 100)
              )}
            </sub>{" "}
            &nbsp;
            <span className="color_r"> {DisplayProd.discountPercentage}% </span>
            offer
          </div>
      ...
      >
        <div className="d-flex g2 align-center">
          ...
          <div className="view_layer">
            ...
            <div className=" d-flex">
              <span>₹ {DisplayProd.price}</span> &nbsp;
              <span className="text-line light_black fs-09">
                {" "}
                ₹
                {parseInt(
                  DisplayProd?.price /
                    (1 - DisplayProd.discountPercentage / 100)
                )}
              </span>{" "}
              &nbsp;
              <span className="color_r">
                {" "}
                {DisplayProd.discountPercentage}%{" "}
              </span>
              &nbsp;<span>offer</span>
            </div>
